...
function statements(trailing) {
	...
	return Object.fromEntries([
		...
		[rn('case'),        $ => prec(2,seq(
			...
			optional(seq(
				// tico: FPC accepts `otherwise` as a synonym for `else` here.
		...
		// tico: labels may be unsigned integers as well as identifiers.
		[rn('goto'),        $ => seq($.kGoto, choice($.identifier, $.literalNumber), ...semicolon)],
...
module.exports = grammar({
	...
	rules: {
		...
		program:            $ => seq(
			// tico: standard Pascal's program parameters, program X(input, output);
			$.kProgram, $.moduleName, optional(seq('(', delimited($.identifier), ')')), ';',
		...
		label:           $ => seq(choice($.identifier, $.literalNumber), ':'),
		...
		_statementsTr:   $ => seq(
			...
			// tico: a block may end with a bare label, `Done: end`.
		...
		_literalString:  $ => choice(/'[^']*'/, $.literalChar, $.literalCaret),
		literalChar:     $ => seq('#', $._literalInt),
		// tico: Turbo Pascal control-character literals, ^G or 'x'^M^J. Only
		// valid where a string literal is, so P^ and P^[i] still lex as kHat.
		literalCaret:    $ => token(/\^[A-Za-z@\[\\\]^_]/),
		literalNumber:   $ => choice($._literalInt, $._literalFloat),
		_literalInt:     $ => choice(
			token.immediate(/[-+]?[0-9]+/),
			token.immediate(/\$[a-fA-F0-9]+/),
			// tico: FPC binary (%1010) and octal (&777) literals.
			token.immediate(/%[01]+/),
			token.immediate(/&[0-7]+/)
		),
		// tico: the exponent marker is case-insensitive (1.5E10, 1E3).
		...
		declVar:         $ => seq(
			...
			field('type', $.type),
			optional(choice(
				// tico: Turbo Pascal's absolute $SEG:$OFS form as well as absolute NAME.
				seq($.kAbsolute, $._ref, optional(seq(':', $._ref))),
		...
		declLabel:       $ => field('name', choice($.identifier, $.literalNumber)),
		...
		// tico: subrange types, `1..10`, `'a'..'z'`, `-MaxInt..MaxInt`,
		// `Low(T)..High(T)`, on their own or after `set of`. The bounds are
		// deliberately not full expressions: an identifier or a `(` in type
		// position would otherwise be ambiguous with a qualified type name
		// or an enum declaration until the `..` shows up.
		declSubrange:    $ => seq(
			field('lo', $.subrangeBound), '..', field('hi', $.subrangeBound)
		),
		subrangeBound:   $ => choice(
			$.literalNumber,
			// Not $.literalString: that admits literalCaret, whose token
			// would then be valid in type position and eat the ^ of ^TNode.
			alias(choice(/'[^']*'/, $.literalChar), $.literalString),
			seq(optional(choice($.kSub, $.kAdd)), $.identifier),
			seq($.identifier, '(', delimited($._expr), ')')
		),
		declArray:       $ => seq(
			optional($.kPacked),
			$.kArray,
